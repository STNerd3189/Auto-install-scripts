...
sudo dnf install -y kernel kernel-devel kernel-headers kernel-tools

#Add RPMFusion repository for gaming and proprietary packages
sudo dnf install -y https://mirrors.rpmfusion.org/free/fedora/rpmfusion-free-release-$(rpm -E %fedora).noarch.rpm https://mirrors.rpmfusion.org/nonfree/fedora/rpmfusion-nonfree-release-$(rpm -E %fedora).noarch.rpm

#Install GNOME and desktop
sudo dnf install -y gnome-shell gnome-tweaks

#Install packages
sudo dnf install -y vim curl ruby fish p7zip exa steam lutris wine proton-gamemode mangohud goverlay ardour obs-studio discord
...
#Remove bloatware
sudo dnf remove -y libreoffice firefox
